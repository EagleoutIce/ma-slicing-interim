...
\def\rhead#1{\textcolor{shadeA}{\bfseries#1}}
...
% TODO: unify
\newcommand<>\citeon[1]{\begin{tikzpicture}[overlay,remember picture]
  \only#2{\node[FCite] at(current page.south west) {\cite{#1}~\citeauthor{#1}, \citetitle{#1} (\citeyear{#1})};}
\end{tikzpicture}}

\def\Content#1#2{#1\\[-.6ex]\small\textit{\color{gray}#2}}
