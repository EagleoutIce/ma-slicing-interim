...
\newcommand\BraceRight[4][2mm]{\draw[line cap=round, gray,decorate,decoration={brace}] ([shift={(#1,.5mm)}]#2.north west-|#3.east) to[edge node={node[right=1mm,font=\small] {#4}}] ([shift={(#1,-.5mm)}]#3.south east);}
...
\begin{frame}[t]{Existing (Analysis) Support for R}
   \vspace*{-3mm}\begin{itemize}
      \itemsep10pt
      \item<2-> [CITE CITE CITE languageserver and code depends]%\raisebox{-.248\height}{\includegraphics[height=1.5\baselineskip]{logos/RStudio-Logo-flat.pdf}}
      RStudio IDE\quad \LinkMark{\link{https://posit.co/products/open-source/rstudio/}{posit.co}}
      \begin{itemize}
      ...
      \end{itemize}
      \item<4-> R~languageserver\quad \LinkMark{\link{https://github.com/REditorSupport/languageserver/}{github.com/REditorSupport}}\begin{itemize}
...
\column{.6\linewidth}
\begin{itemize}[<+(1)->]
   \item<8-> CodeDepends\quad \LinkMark{\link{https://github.com/duncantl/CodeDepends}{github.com/duncantl/CodeDepends}} \begin{itemize}
...
\begin{tikzpicture}[@O]
   ...
   \onslide<12-|handout:2>{
      \BraceRight[3.1cm]{r-lang-start}{r-lang-end}{Based on XPath-expressions}
   }
