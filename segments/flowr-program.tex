...
\newsavebox\FullUseGraph \savebox\FullUseGraph{\WithGraph{%
   ...
   \node[u,below=2mm] (x) at (y.south) {x\textsubscript{2}};
   \node[d,left=4mm] (z) at (current bounding box.west) {z\textsubscript{1}};
   \node[d,right=4mm] (xb) at(x.east) {x\textsubscript{1}};
   \node[d,right=4mm] (yb) at(y.east) {y\textsubscript{1}};
   \draw[Kite-] (z) -- (x);
   \draw[Kite-] (z) -- (y);
