...
\pgfplotsset{
   HistAxis/.style={
      ymin=0, height=7.5cm,width=5cm, ymax=\YLimiter,%  xmax=1000,
      ...
      y tick label style={font=\sffamily\footnotesize,yshift=1.75mm},
...
#3
\end{axis}#4
\node[gray,yshift=2mm,below right] (@) at(current bounding box.south west) {\footnotesize\strut\textsf{user}};
\node[gray,left] at (@.east-|current bounding box.south east) {\footnotesize\strut\textsf{package}};
\end{tikzpicture}%
}
